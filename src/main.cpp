    ...
    // Compare results with the reference NTT implementation
    {
        ...
        for (size_t i = 0; i < t.n; ++i) {
            if (outData_acc[i] != reference_data[i]) {
                std::cerr << "Mismatch at index " << i << ": FPGA=" << outData_acc[i]
        ...
        if (success) {
            std::cout << "FPGA kernel test passed." << std::endl;
        } else {
            std::cerr << "FPGA kernel test failed." << std::endl;
        }
    }
}
...
int main() {
    ...
    for (size_t i = 0; i < NUM_OF_TEST_CASES; ++i) {
        ...
        if (test_correctness(&tests[i]) != SUCCESS) {
            std::cerr << "Test case " << i + 1 << " failed on CPU!" << std::endl;
        } else {
            std::cout << "Test case " << i + 1 << " passed on CPU." << std::endl;
        }

        // Test the FPGA kernel implementation
        test_fpga_kernel(q, tests[i]);
    }
